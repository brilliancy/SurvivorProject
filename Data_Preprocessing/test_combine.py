	...
	******PROBLEM: makes an assumption that the dictionary is in order, however it isn't****
	...
	for season,episode in episode_ord_dict.items():
		for episode_num in range(1,episode+1): #***removed episode+1
			...
			MiddleDf = pd.read_csv(filename)
			# adding season/episode col 
			'''
			*******Episode ID and Episode isn't working right
			'''
			MiddleDf['Season'] = season 
			MiddleDf['Episode'] = episode_num 
			#adding episode id
			EpisodeID_num += 1
			MiddleDf['EpisodeId'] = EpisodeID_num
